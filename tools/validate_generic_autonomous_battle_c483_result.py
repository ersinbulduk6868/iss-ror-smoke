...
from validate_generic_autonomous_battle_c482_result import collision_role_runtime_proof
from validate_generic_heterogeneous_cross_gate_v1 import heterogeneous_cross_gate_proof


def drive_direction_runtime_proof(log_path: str) -> dict[str, object]:
    rows = _json_markers(log_path)
    ready = [
        r for r in rows
        if r.get("marker") == "GENERIC_AUTONOMOUS_BATTLE_C483_ENGINEERING_READY"
    ]
    assert len(ready) == 1, ("C483_ENGINEERING_READY_COUNT", len(ready))
    row = ready[0]
    assert row.get("candidate") == "ISS_GENERIC_BATTLE_RUNTIME_V1_CANDIDATE_4_8_3_GENERIC_AUTONOMOUS_BATTLE", row
    assert row.get("mechanism") == "GENERIC_CHASSIS_FORWARD_TO_BLENDER_MOTOR_SIGN_V1", row
    assert row.get("canonicalForwardCommandPreserved") is True, row
    assert row.get("blenderMotorSignAdapted") is True, row
    assert row.get("sameMappingAcrossAssets") is True, row
    assert row.get("c482CollisionRolesPreserved") is True, row
    assert row.get("c481DeferredHandoffProgressPreserved") is True, row
    assert row.get("c480ApproachSemanticTransactionPreserved") is True, row
    assert row.get("g05NativeSolverFinalAuthorityPreserved") is True, row
    ...
    assert row.get("damageThresholdAwareControl") is False, row
    assert row.get("targetToughnessAwareControl") is False, row
    assert row.get("desiredImpactSpeedControl") is False, row
    assert row.get("desiredImpactEnergyControl") is False, row
    ...
    assert row.get("perVideoTrajectoryEngineering") is False, row
    assert row.get("fixedWorldCoordinates") is False, row
    assert row.get("actorPoseOrVelocityMutation") is False, row
    assert row.get("fixtureBattlePlanChanged") is False, row
    return {
        "driveDirectionRuntimeReceipt": "PASS",
        "driveDirectionMechanism": row.get("mechanism"),
...
def main() -> None:
    ...
    drive_direction = drive_direction_runtime_proof(a.hetero_log)
    ...
    print(json.dumps({
        ...
        "mechanism": "GENERIC_CHASSIS_FORWARD_TO_BLENDER_MOTOR_SIGN_V1",
