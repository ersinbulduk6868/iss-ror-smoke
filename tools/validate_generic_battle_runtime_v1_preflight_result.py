...
EXPECTED_RUNTIME = "ISS_GENERIC_BATTLE_RUNTIME_V1_CANDIDATE_3_HARDENED"
...
def main() -> None:
    ...
    coast_samples = [
        row
        for row in samples
        if row.get("eventId") == "evt-impact"
        and row.get("actorId") == "actor_alpha"
        and row.get("controllerAuthority") == "COAST"
    ...
    print(
        json.dumps(
            {
                "marker": "GENERIC_BATTLE_RUNTIME_CANDIDATE3_MACHINE_PREFLIGHT",
                "status": "PASS",
