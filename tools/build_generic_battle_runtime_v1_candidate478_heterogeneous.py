...
def main() -> None:
    ...
    p.add_argument("--bulldozer-ready-master", required=True)
    a = p.parse_args()

    bulldozer = Path(a.bulldozer_ready_master).expanduser().resolve()
    if not bulldozer.is_file():
        raise RuntimeError(f"C478_BULLDOZER_READY_MASTER_MISSING:{bulldozer}")
    bulldozer_sha = sha256_file(bulldozer)
    if bulldozer_sha != EXPECTED_BULLDOZER_READY_MASTER_SHA:
        raise RuntimeError(
            f"C478_BULLDOZER_READY_MASTER_SHA_MISMATCH:{bulldozer_sha}:{EXPECTED_BULLDOZER_READY_MASTER_SHA}"
        )
    ...
    beta.update({
        ...
        "readyMasterSha256": EXPECTED_BULLDOZER_READY_MASTER_SHA,
        ...
        "physicsManifest": {
            "dynamicPhysics": "RC2_REAL_L4_PASS",
            "rigidBodies": 5,
            "collisionShapes": 13,
            "fixedStructuralJoints": 4,
        },
        "runtimeProfile": {
    ...
    for forbidden in (
        ...
        "maxYawRateRadS", "toughnessJPerKg", "forwardAxis",
        ...
        if forbidden in beta_rp:
            raise RuntimeError(f"C478_HEAVY_PROFILE_ACCEPTANCE_TUNING_FORBIDDEN:{forbidden}")
    ...
    print(json.dumps({
        ...
        "bulldozerSourceSha256": EXPECTED_BULLDOZER_SOURCE_SHA,
        "bulldozerReadyMasterSha256": bulldozer_sha,
