...
from blender.iss_battle_runtime_tactics_v4 import (
    BATTLE_SIGNAL_SCOPE,
...
def _static_scope() -> None:
    for path in SOURCES:
        ...
        for token in FORBIDDEN_ASSET_TOKENS:
            if token in lowered:
                raise SystemExit(f"C462_ASSET_SPECIFIC_TOKEN_FORBIDDEN:{path.name}:{token}")
...
def main() -> None:
    ...
    retry_memory = TacticalMemory()
    initial_damage_intent = GenericBattleTacticalPlanner.decide(
        retry_memory,
        _obs(frame=2, damage_required=True, current_event_contact_count=0, current_event_damage_count=0),
        symmetry_bias=1.0,
    )
    nonproductive = GenericBattleTacticalPlanner.decide(
        retry_memory,
        _obs(frame=30, contact_count=1, surface_gap_m=0.05,
             damage_required=True, current_event_contact_count=1, current_event_damage_count=0),
        symmetry_bias=1.0,
    )
    assert nonproductive.mode == "BREAK_CONTACT"
    separated_retry = GenericBattleTacticalPlanner.decide(
        retry_memory,
        _obs(frame=31, contact_count=1,
             surface_gap_m=retry_memory.separation_required_m + 0.4,
             damage_required=True, current_event_contact_count=1, current_event_damage_count=0),
        symmetry_bias=1.0,
    )
    assert separated_retry.mode == "REPOSITION"
    retry_attack = GenericBattleTacticalPlanner.decide(
        retry_memory,
        _obs(frame=retry_memory.reposition_until_frame + 1, contact_count=1,
             surface_gap_m=retry_memory.separation_required_m + 0.8,
             damage_required=True, current_event_contact_count=1, current_event_damage_count=0),
        symmetry_bias=1.0,
    )
    assert retry_attack.mode == "ENGAGE" and retry_attack.speed_scale > initial_damage_intent.speed_scale

    satisfied_memory = TacticalMemory()
    satisfied = GenericBattleTacticalPlanner.decide(
        satisfied_memory,
        _obs(frame=2, damage_required=True, current_event_contact_count=1, current_event_damage_count=1),
        symmetry_bias=1.0,
    )
    assert satisfied.mode == "ENGAGE"
    assert satisfied.speed_scale == initial_damage_intent.speed_scale
    ...
    print(json.dumps({
        "marker": "GENERIC_AUTONOMOUS_BATTLE_C462_PROPERTY_ACCEPTANCE",
        "status": "PASS",
        ...
        "damageSatisfiedStopsRetryPressure": True,
