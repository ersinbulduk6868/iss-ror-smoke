...
def sample(frame: int = 10, event: str = "evt-a", actor: str = "actor-a", target: str = "actor-b") -> PrecontactRealizationSample:
    return PrecontactRealizationSample(
        frame=frame, event_id=event, actor_id=actor, target_id=target,
        forward_speed_mps=1.4, closing_speed_mps=1.3, capability_floor_mps=1.0,
        effective_gap_m=0.30, handoff_gap_m=0.10,
    )
...
def main() -> None:
    ...
    assert precontact_sample_eligible(
        contact_approach_active=True, recovery_active=False,
        forward_speed_mps=1.4, closing_speed_mps=1.3, capability_floor_mps=1.0,
        effective_gap_m=0.30, handoff_gap_m=0.10,
    ) is True
    assert precontact_sample_eligible(
        contact_approach_active=True, recovery_active=True,
        forward_speed_mps=1.4, closing_speed_mps=1.3, capability_floor_mps=1.0,
        effective_gap_m=0.30, handoff_gap_m=0.10,
    ) is False
    assert precontact_sample_eligible(
        contact_approach_active=True, recovery_active=False,
        forward_speed_mps=1.4, closing_speed_mps=1.3, capability_floor_mps=1.0,
        effective_gap_m=0.10, handoff_gap_m=0.10,
    ) is False

    key = ("evt-a", "actor-a", "actor-b")
    s = sample()
    assert previous_frame_sample_valid_for_handoff(
        s, transaction_key=key, decision_frame=11, current_capability_floor_mps=1.0,
        current_closing_speed_mps=0.01, controller_handoff_requested=True,
    ) is True
    for decision in (10, 12, 20):
        assert previous_frame_sample_valid_for_handoff(
            s, transaction_key=key, decision_frame=decision, current_capability_floor_mps=1.0,
            current_closing_speed_mps=0.01, controller_handoff_requested=True,
        ) is False
    for wrong in (("evt-b", "actor-a", "actor-b"), ("evt-a", "actor-a", "actor-c")):
        assert previous_frame_sample_valid_for_handoff(
            s, transaction_key=wrong, decision_frame=11, current_capability_floor_mps=1.0,
            current_closing_speed_mps=0.01, controller_handoff_requested=True,
        ) is False
    assert previous_frame_sample_valid_for_handoff(
        s, transaction_key=key, decision_frame=11, current_capability_floor_mps=1.0,
        current_closing_speed_mps=-0.001, controller_handoff_requested=True,
    ) is False
    assert previous_frame_sample_valid_for_handoff(
        s, transaction_key=key, decision_frame=11, current_capability_floor_mps=1.0,
        current_closing_speed_mps=0.01, controller_handoff_requested=False,
    ) is False
    assert previous_frame_sample_valid_for_handoff(
        s, transaction_key=key, decision_frame=11, current_capability_floor_mps=1.5,
        current_closing_speed_mps=0.01, controller_handoff_requested=True,
    ) is False
    ...
    for required in (
        ...
        "IMMEDIATELY_PREVIOUS_PRECONTACT_FRAME",
    ...
    for forbidden in (
        ...
        "handoff_to_solver",
        "bugatti", "bulldozer", "min_closing_speed_mps", "min_damage_severity",
        "desiredimpactspeedmps", "desiredimpactenergyj", "trajectorypoints", "waypoints",
    ...
    print(json.dumps({
        ...
        "g05ThresholdImported":False,"assetSpecificCode":False,
        "perAssetTacticalTuning":False,"perVideoTrajectoryEngineering":False,
        "masterPlanAligned":True,"gateClosed":False
    },sort_keys=True))
