...
EXPECTED_BULLDOZER_READY_MASTER_SHA = "8ab94079ad22974f037bb01732365ffafdf2e107edd19865d145b9e33cc763c6"
...
def profile_proof(base_req: dict, hetero_req: dict) -> dict[str, object]:
    ...
    assert str(b.get("readyMasterSha256") or "").lower() == EXPECTED_BULLDOZER_READY_MASTER_SHA, b
    ...
    return {
        ...
        "explicitForwardAxisInjected": False,
...
def orientation_proof(log_path: str) -> dict[str, object]:
    rows: list[dict] = []
    for raw in Path(log_path).read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        try:
            ...
            continue
        if row.get("marker") == "G04_FORWARD_AXIS_RESOLVED_FROM_PHYSICS_SEMANTICS":
            rows.append(row)
    beta = [r for r in rows if r.get("entityId") == "actor_beta"]
    assert beta, "C478_PHYSICS_SEMANTIC_ORIENTATION_MARKER_MISSING"
    row = beta[-1]
    assert row.get("axis") == "X", row
    assert row.get("referenceSource") == "PHYSICS_CHASSIS_SEMANTIC", row
    assert "blade" in (row.get("frontAnchors") or []), row
    assert any(str(x).startswith("chassis") for x in (row.get("chassisAnchors") or [])), row
    assert row.get("assetIdentityBranch") is False, row
    assert row.get("sourceShaBranch") is False, row
    assert row.get("perAssetOrientationOverride") is False, row
    return {
        "physicsSemanticOrientationFallback": "PASS",
        "resolvedAxis": row.get("axis"),
        "referenceSource": row.get("referenceSource"),
...
def main() -> None:
    ...
    orientation = orientation_proof(a.hetero_log)
    ...
    print(json.dumps({
        ...
        "mechanism": "G04_PHYSICS_SEMANTIC_ORIENTATION_FALLBACK_V1",
        "executionProfile": "NVIDIA_L4",
        "c474TacticalRuntimePreserved": True,
        "orientationInterfaceGapClosed": "PASS",
