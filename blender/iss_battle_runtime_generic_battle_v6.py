...
BATTLE_CONTROL_MODEL = "ISS_GENERIC_AUTONOMOUS_BATTLE_CONTROL_V6"
SURFACE_GAP_GOAL_MODEL = "LIVE_SUPPORT_RADIUS_SURFACE_GAP_GOAL_V1"
...
_handoff_latches: dict[tuple[str, str], SolverHandoffLatch] = {}
...
def _safe_horizontal(vector: Vector, fallback: Vector) -> Vector:
    out = Vector(vector)
    out.z = 0.0
    if out.length <= 1.0e-8:
        out = Vector(fallback)
        out.z = 0.0
    if out.length <= 1.0e-8:
...
def _surface_gap_goal(
    actor: Any,
    target: Any,
    desired_surface_gap_m: float,
    lateral_bias: float,
) -> Vector:
    """Convert a desired surface gap into a live center goal.

    The goal is recomputed from current chassis geometry every frame. It uses no
    asset identity, fixed world point, scripted collision coordinate or impact target.
    """
    actor_pos = actor.chassis.matrix_world.translation.copy()
    target_pos = target.chassis.matrix_world.translation.copy()
    radial = _safe_horizontal(actor_pos - target_pos, Vector((-1.0, 0.0, 0.0)))

    target_q = target.chassis.matrix_world.to_quaternion()
    lateral = _safe_horizontal(target_q @ Vector((0.0, 1.0, 0.0)), Vector((0.0, 1.0, 0.0)))
    radial = _safe_horizontal(radial + lateral * float(lateral_bias) * 0.32, radial)

    actor_toward_target = -radial
    target_toward_actor = radial
    actor_support = control_v2._support_radius(actor, actor_toward_target)
    target_support = control_v2._support_radius(target, target_toward_actor)
    center_separation = (
        max(0.0, float(desired_surface_gap_m))
        + max(0.0, float(actor_support))
        + max(0.0, float(target_support))
    )
    return target_pos + radial * center_separation


def _goal_for_tactical(
    actor: Any,
    target: Any,
    event: Any,
    tactical: Any,
    ...
    if tactical.mode in {"OPEN_DISTANCE", "BREAK_CONTACT", "REPOSITION"}:
        gap = max(0.0, float(tactical.stand_off_surface_gap_m))
        lateral = float(tactical.lateral_offset_scale) if tactical.mode == "REPOSITION" else 0.0
        return _surface_gap_goal(actor, target, gap, lateral)
    return control_v2._target_goal(actor, target, event, tactical, actors)
...
def set_controls(
    ...
    *,
    ...
    for entity, actor in actors.items():
        ...
        forward = _safe_horizontal(q @ Vector((1.0, 0.0, 0.0)), Vector((1.0, 0.0, 0.0)))
        ...
        if target is not None:
            ...
            gap = center_distance = closing = direct_heading_error = 0.0
        ...
        tactical_obs = TacticalObservation(
            ...
            heading_error_rad=float(direct_heading_error),
            contention=float(control_v2._contention_score(entity, actor, actors, event.target_id)),
            own_integrity=float(actor.state.structural_integrity),
            own_drive_efficiency=float(actor.state.drive_efficiency),
            own_disabled=bool(actor.state.disabled),
            target_integrity=target_integrity,
            target_drive_efficiency=target_efficiency,
            target_disabled=target_disabled,
            own_max_speed_mps=float(actor.profile.max_speed_mps),
            own_max_reverse_mps=float(actor.profile.max_reverse_mps),
            own_yaw_rate_rad_s=float(actor.profile.max_yaw_rate_rad_s),
            own_acceleration_mps2=float(actor.profile.acceleration_mps2),
            own_braking_mps2=float(actor.profile.braking_mps2),
            own_length_m=max(0.25, float(actor.dimensions.x)),
            own_width_m=max(0.25, float(actor.dimensions.y)),
            target_length_m=max(0.25, target_length),
            target_width_m=max(0.25, target_width),
            contact_count=int(actor_contact_signal),
            damage_count=int(actor_damage_signal),
            own_damage_event_count=len(actor.state.damage_events),
            target_damage_event_count=int(target_damage_count),
            last_own_impact_frame=actor.state.last_impact_frame,
            last_target_impact_frame=target_last_impact,
        )
        ...
        if target is not None:
            goal_point = _goal_for_tactical(actor, target, event, tactical, actors)
        else:
        ...
        autonomy_obs = AutonomyObservation(
            frame=int(frame),
            fps=int(program.fps),
            distance_m=max(0.0, float(target_vector.length)),
            surface_gap_m=float(gap),
            heading_error_rad=float(heading_error),
            forward_speed_mps=forward_speed,
            closing_speed_mps=float(closing),
            contention=float(tactical_obs.contention),
            ...
            speed_intent=str(tactical.speed_intent),
        )

        handoff_key = (event.event_id, entity)
        latch = _handoff_latches.get(handoff_key)
        handoff_latched = False
        handoff_transition = False
        handoff_release_reason = None
        handoff_release_gap = 0.0

        if latch is not None:
            handoff_decision = decide_solver_handoff(
                latch,
                current_contact_count=int(state.contact_count),
                surface_gap_m=float(gap),
                closing_speed_mps=float(closing),
                characteristic_length_m=max(0.25, float(actor.dimensions.x)),
            )
            handoff_release_gap = float(handoff_decision.release_gap_m)
            if handoff_decision.hold:
                actor.rig.coast()
                hardened._cutoff_frames[handoff_key] = int(frame)
                handoff_latched = True
                motor_authority = "COAST"
                left = right = 0.0
                command_mode = "CONTACT_HANDOFF_LATCH"
                command_reason = handoff_decision.reason
                command_speed = command_yaw = 0.0
                replan_triggered = False
                progress_timeout = ClosedLoopGoalController.progress_timeout_frames(autonomy_obs)
                handoff_gap = float(latch.handoff_gap_m)
            else:
                handoff_release_reason = handoff_decision.reason
                handoff_transition = True
                _handoff_latches.pop(handoff_key, None)
                marker(
                    "GENERIC_SOLVER_HANDOFF_RELEASED",
                    frame=int(frame),
                    eventId=event.event_id,
                    actorId=entity,
                    reason=handoff_decision.reason,
                    surfaceGapM=round(float(gap), 6),
                    closingSpeedMps=round(float(closing), 6),
                    releaseGapM=round(float(handoff_decision.release_gap_m), 6),
                    model=SOLVER_HANDOFF_LATCH_MODEL,
                )

        if not handoff_latched:
            if tactical.speed_intent == "BRAKE":
                ...
                if command.motor_authority == "COAST":
                    ...
                    if command.mode == "CONTACT_HANDOFF":
                        hardened._cutoff_frames[handoff_key] = int(frame)
                        _handoff_latches[handoff_key] = SolverHandoffLatch(
                            start_frame=int(frame),
                            contact_count_at_latch=int(state.contact_count),
                            handoff_gap_m=float(handoff_gap),
                            last_surface_gap_m=float(gap),
                        )
                        handoff_latched = True
                        handoff_transition = True
                        handoff_release_gap = max(
                            float(handoff_gap) * 2.5,
                            max(0.25, float(actor.dimensions.x)) * 0.06,
                            0.15,
                        )
                        marker(
                            "GENERIC_SOLVER_HANDOFF_LATCHED",
                            frame=int(frame),
                            eventId=event.event_id,
                            actorId=entity,
                            contactCountAtLatch=int(state.contact_count),
                            surfaceGapM=round(float(gap), 6),
                            closingSpeedMps=round(float(closing), 6),
                            handoffGapM=round(float(handoff_gap), 6),
                            model=SOLVER_HANDOFF_LATCH_MODEL,
                        )
                else:
        ...
        if replan_triggered:
            ...
            marker(
                ...
                model="ISS_WORLD_STATE_REPLANNER_V1",
            )
        ...
        if previous_tactical != tactical.mode:
            ...
            marker(
                "GENERIC_BATTLE_TACTIC_CHANGED",
                frame=int(frame),
                eventId=event.event_id,
                actorId=entity,
                previousMode=previous_tactical,
                mode=tactical.mode,
                reason=tactical.reason,
                cycle=int(tactical.cycle),
        ...
        if (
            tactical.transition
            or replan_triggered
            or handoff_transition
            or handoff_latched
            or frame == event.start_frame
            or frame % max(1, int(program.fps) // 4) == 0
        ):
            row = {
                "frame": int(frame),
                "eventId": str(event.event_id),
                "actorId": entity,
                "storyPhase": str(event.phase),
                "storyTactic": str(event.tactic),
                "tacticalMode": tactical.mode,
                "tacticalReason": tactical.reason,
                "battleCycle": int(tactical.cycle),
                "speedIntent": tactical.speed_intent,
                "speedScale": float(tactical.speed_scale),
                "contactCommit": bool(tactical.contact_commit),
                "motorAuthority": motor_authority,
                "controllerMode": command_mode,
                "controllerReason": command_reason,
                "leftMps": float(left),
                "rightMps": float(right),
                "forwardCommandMps": command_speed,
                "yawRateCommandRadS": command_yaw,
                "observation": {
                    "surfaceGapM": float(gap),
                    "centerDistanceM": float(center_distance),
                    "closingSpeedMps": float(closing),
                    "headingErrorRad": float(heading_error),
                    "rawForwardHeadingErrorRad": float(raw_heading_error),
                    "contention": float(tactical_obs.contention),
                    "ownIntegrity": float(actor.state.structural_integrity),
                    "ownDriveEfficiency": float(actor.state.drive_efficiency),
                ...
                "policy": {
                    ...
                    "engagementRunwayRequiredM": float(tactical_memory.engagement_runway_required_m),
                    "engagementRunwayArmed": bool(tactical_memory.engagement_runway_armed),
                    "standOffSurfaceGapM": float(tactical.stand_off_surface_gap_m),
                    "engagementRunwayModel": ENGAGEMENT_RUNWAY_MODEL,
                    "surfaceGapGoalModel": SURFACE_GAP_GOAL_MODEL,
                    "solverHandoffLatchModel": SOLVER_HANDOFF_LATCH_MODEL,
                    "solverHandoffLatched": bool(handoff_latched),
                    "solverHandoffReleaseReason": handoff_release_reason,
                    "solverHandoffReleaseGapM": float(handoff_release_gap),
...
def write_evidence(output_dir: Path) -> Path:
    ...
    path = output_dir / "generic-autonomous-battle-v6-evidence.json"
    path.write_text(json.dumps(evidence, indent=2, sort_keys=True), encoding="utf-8")
    marker(
        "GENERIC_AUTONOMOUS_BATTLE_V6_EVIDENCE_WRITTEN",
        path=str(path),
        sampleCount=len(_tactical_samples),
        modeCount=len(mode_counts),
        actorCount=len(actors),
        model=BATTLE_CONTROL_MODEL,
    )
    return path
