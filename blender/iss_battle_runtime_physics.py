...
TERMINAL_EVENT_STATES = {"SUCCEEDED", "SETTLED", "OBSERVED", "FAILED", "FAILED_DEPENDENCY"}
...
class DriveRig:
    ...
    def command(self, left_mps: float, right_mps: float, impulse_scale: float = 1.0) -> None:
        ...
        impulse = max(0.0, self.max_motor_impulse * float(impulse_scale))
    ...
    def coast(self) -> None:
        """Remove motor authority so the rigid-body solver owns the contact response."""
        for obj in self.motors_left + self.motors_right:
            c = obj.rigid_body_constraint
            c.motor_ang_target_velocity = 0.0
            c.motor_ang_max_impulse = 0.0

    def brake(self, strength: float = 1.0) -> None:
        self.command(0.0, 0.0, max(1.0, float(strength) * 1.5))
    ...
    def zone_world(self, zone: str | None) -> Vector:
        ...
        if key not in self.prototype.zones:
            raise BlenderBattleRuntimeError(
                f"TARGET_SEMANTIC_ZONE_UNRESOLVED:{self.profile.entity_id}:{key}"
            )
...
class PendingContact:
    ...
    controller_cutoff_observed: bool = False
    ...
    for axle_label, sx in (("F", 1.0), ("R", -1.0)):
        for side_label, sy in (("L", 1.0), ("R", -1.0)):
            ...
            add_rigid_body(
                ...
                mass=wheel_mass,
                shape="CYLINDER",
                friction=max(1.1, profile.friction),
                restitution=0.01,
            )
            hinge = add_constraint(
                f"ISS_HINGE_{norm(entity)}_{axle_label}{side_label}",
                "HINGE",
                center,
                axis,
                chassis,
            ...
            motor = add_constraint(
                f"ISS_MOTOR_{norm(entity)}_{axle_label}{side_label}",
                "MOTOR",
                center,
                axis,
                chassis,
            ...
            hinges.append(hinge)
            if side_label == "L":
                wheels_left.append(wheel)
                motors_left.append(motor)
            else:
                wheels_right.append(wheel)
                motors_right.append(motor)
    return DriveRig(
        chassis,
        wheels_left,
        wheels_right,
...
def create_runtime_actor(
    ...
    wheel_radius = clamp(
        float(dims.z) * 0.24,
        0.26,
        min(0.90, float(dims.x) * 0.16),
    )
    chassis_height = max(0.35, float(dims.z) * 0.62)
    chassis_center_z = wheel_radius + chassis_height * 0.52
    chassis = cube(
        f"ISS_PHYSICS_{norm(profile.entity_id)}",
        (spawn[0], spawn[1], chassis_center_z),
        (
            max(0.55, dims.x * 0.91),
            max(0.45, dims.y * 0.84),
            chassis_height,
        ),
    )
    ...
    add_rigid_body(
        ...
        shape="BOX",
        friction=max(0.15, profile.friction * 0.22),
        restitution=profile.restitution,
    )
    instance = bpy.data.objects.new(f"ISS_VISUAL_{norm(profile.entity_id)}", None)
    bpy.context.scene.collection.objects.link(instance)
    instance.instance_type = "COLLECTION"
    instance.instance_collection = proto.collection
    instance.parent = chassis
    visual_offset = Vector((0.0, 0.0, -chassis_center_z))
    instance.location = visual_offset
    rig = create_drive_rig(profile.entity_id, chassis, dims, profile, yaw)
    return RuntimeActor(
        binding,
        profile,
        ActorState(profile.entity_id),
        proto,
        chassis,
        instance,
        visual_offset,
...
def obb_axes(actor: RuntimeActor) -> tuple[Vector, Vector]:
    ...
    x = q @ Vector((1.0, 0.0, 0.0))
    y = q @ Vector((0.0, 1.0, 0.0))
    x.z = 0.0
    y.z = 0.0
    if x.length < 1e-6 or y.length < 1e-6:
        raise BlenderBattleRuntimeError("OBB_AXIS_DEGENERATE")
    return x.normalized(), y.normalized()


def obb_overlap_2d(a: RuntimeActor, b: RuntimeActor) -> bool:
    pa = a.chassis.matrix_world.translation
    pb = b.chassis.matrix_world.translation
    delta = pb - pa
    ax, ay = obb_axes(a)
    bx, by = obb_axes(b)
    ha = (
        float(a.chassis.dimensions.x) * 0.5,
        float(a.chassis.dimensions.y) * 0.5,
    )
    hb = (
        float(b.chassis.dimensions.x) * 0.5,
        float(b.chassis.dimensions.y) * 0.5,
    )
    for axis in (ax, ay, bx, by):
        ...
        if center_distance > ra + rb + 0.06:
            return False
    return abs(pa.z - pb.z) <= (
        float(a.chassis.dimensions.z) + float(b.chassis.dimensions.z)
    ) * 0.55 + 0.12


def signed_heading_error(forward: Vector, desired: Vector) -> float:
    f = Vector((forward.x, forward.y, 0.0))
    d = Vector((desired.x, desired.y, 0.0))
    if f.length < 1e-5 or d.length < 1e-5:
        return 0.0
    f.normalize()
    d.normalize()
    return math.atan2(
        f.x * d.y - f.y * d.x,
        clamp(f.dot(d), -1.0, 1.0),
    )


def choose_initial_yaw(
    entity: str,
...
def event_dependency_ready(
    event: RuntimeEvent,
    states: dict[str, EventState],
) -> bool:
    return all(
        states[d].status in {"SUCCEEDED", "SETTLED", "OBSERVED"}
        for d in event.dependencies
    )


def active_event_for_actor(
    entity: str,
    frame: int,
    program: BattleProgram,
    states: dict[str, EventState],
    ...
    for event in program.events:
        ...
        grace = (
            program.fps
            if program.policies.get("replanOnPhysicalImpossibility", True)
            and event.requires_contact
            else 0
        )
        if (
            event.start_frame
...
def event_target_point(
    ...
    dx, dy = TargetingPlanner.approach_offset(
        event.tactic,
        attacker_index,
        attacker_count,
        float(target.dimensions.x),
        float(target.dimensions.y),
    )
    if state.attempts % 2:
        dy *= -1.0
    target_q = target.chassis.matrix_world.to_quaternion()
    offset_world = target_q @ Vector((dx, dy, 0.0))
    attacker_pos = attacker.chassis.matrix_world.translation
    distance = (zone - attacker_pos).length
    commit_distance = max(3.0, float(target.dimensions.x) * 1.05)
    if event.tactic in {"RAM", "COUNTER"} or distance <= commit_distance:
        return zone
...
def drive_command(
    ...
    desired = target_point - pos if target_point is not None else forward
    distance = desired.length
    error = signed_heading_error(forward, desired)
    ...
    if event.tactic == "REVERSE" or event.speed_intent == "REVERSE":
        ...
        factor = {
            ...
            "COUNTER": 0.78,
            "RAM": 0.88,
        }.get(event.tactic, 0.66)
        if event.speed_intent == "SUSTAIN":
            factor *= 0.82
        target_speed = profile.max_speed_mps * factor * efficiency

    fwd_speed = (
        current_velocity.dot(forward.normalized())
    ...
    contact_cutoff = max(
        0.55,
        float(actor.chassis.dimensions.x) * 0.52,
    )
    if event.requires_contact and distance <= contact_cutoff:
        return 0.0, 0.0, {
            "headingError": error,
            "targetSpeed": 0.0,
            "distance": distance,
            "forwardSpeed": fwd_speed,
            "controllerCutoff": 1.0,
            "contactCutoffDistance": contact_cutoff,
        }

    if abs(error) > 1.20:
        pivot = min(
            max(1.0, abs(target_speed) * 0.34),
            profile.max_speed_mps * 0.36,
        ) * (1.0 if target_speed >= 0 else -1.0)
        left, right = -pivot, pivot
    else:
        turn = clamp(
            error / max(0.32, profile.max_yaw_rate_rad_s),
            -1.0,
            1.0,
        )
        left = target_speed * (1.0 - 0.86 * turn)
        right = target_speed * (1.0 + 0.86 * turn)

    max_abs = profile.max_speed_mps * max(0.15, efficiency)
    left = clamp(left, -profile.max_reverse_mps, max_abs)
    right = clamp(right, -profile.max_reverse_mps, max_abs)
    if event.speed_intent in {"BRAKE", "STOP", "SETTLE"}:
        left = right = 0.0
    ...
    return left, right, {
        ...
        "forwardSpeed": fwd_speed,
        "controllerCutoff": 0.0,
        "contactCutoffDistance": contact_cutoff if event.requires_contact else 0.0,
    }
