...
def navigation_preserving_surface_gap(attacker: Any, target: Any) -> tuple[float, float, Any]:
    ...
    # Deliberately preserve the original radial navigation contract. SAT is only
    # consumed by the controller handoff eligibility check below.
...
def progress_owned_autonomy_update(
    ...
    *,
    ...
    try:
        rebase = should_rebase_progress_for_tactical_goal(
        ...
        if rebase:
            ...
            # If a completed tactical separation/reposition plan explicitly hands
            # ownership back to a forward maneuver, an old low-level recovery may
            # not override the new goal. Other recovery behavior remains untouched.
        ...
        if current_effective is not None and collision_proxy_progressed(
            ...
            # Keep evidence useful without logging every physics frame.
...
def main() -> None:
    ...
    # Candidate 472 remains immutable provenance. Replace only its runtime hook
    # functions before candidate472.main() resolves them.
