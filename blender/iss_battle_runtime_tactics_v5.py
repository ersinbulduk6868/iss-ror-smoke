...
ENGAGEMENT_RUNWAY_MODEL = "GEOMETRY_CAPABILITY_SURFACE_GAP_RUNWAY_V1"
...
class TacticalMemory:
    ...
    engagement_runway_required_m: float = 0.0
    engagement_runway_armed: bool = False
...
class TacticalGoal:
    ...
    transition: bool
    cycle: int
    stand_off_surface_gap_m: float = 0.0


class GenericBattleTacticalPlanner:
    """Generic live-state battle policy.

    No asset identity, damage threshold, toughness, desired impact speed/energy,
    fixed collision point, or per-video trajectory enters this planner.
    """
    ...
    @staticmethod
    def engagement_runway_required(obs: TacticalObservation) -> float:
        length_term = 0.55 * (
            max(0.25, obs.own_length_m) + max(0.25, obs.target_length_m)
        )
        width_term = 1.35 * max(
            max(0.25, obs.own_width_m), max(0.25, obs.target_width_m)
        )
        maneuver_s = clamp(
            0.55 + 0.35 / max(0.15, obs.own_yaw_rate_rad_s),
            0.75,
            1.10,
        )
        acceleration_term = (
            0.5 * max(0.1, obs.own_acceleration_mps2) * maneuver_s * maneuver_s
        )
        return clamp(
            max(1.50, length_term, width_term, acceleration_term),
            1.50,
            10.0,
        )

    @staticmethod
    def _recovery_separation_required(obs: TacticalObservation) -> float:
        length_term = 0.18 * (
            max(0.25, obs.own_length_m) + max(0.25, obs.target_length_m)
        )
        width_term = 0.30 * max(
    ...
    def _capability_speed_scale(obs: TacticalObservation) -> float:
        ...
        return clamp(
            (0.45 + 0.35 * drive + 0.20 * integrity) * (0.85 + 0.20 * braking_ratio),
            0.20,
            1.0,
        )
    ...
    def _settle(memory: TacticalMemory) -> None:
        ...
        memory.engagement_runway_required_m = 0.0
    ...
    def _open_distance(
        ...
        transition = GenericBattleTacticalPlanner._set_mode(
            memory, "OPEN_DISTANCE", reason
        ) or transition
        return TacticalGoal(
            mode="OPEN_DISTANCE",
            reason=memory.reason,
            speed_intent="REVERSE",
            speed_scale=clamp(base_scale * 0.74, 0.20, 0.78),
            forward_offset_scale=0.0,
            lateral_offset_scale=memory.flank_bias * 0.10,
            contact_commit=False,
            brake_strength=0.0,
            transition=transition,
            cycle=memory.cycle,
            stand_off_surface_gap_m=max(0.0, memory.engagement_runway_required_m),
        )
    ...
    def decide(
        ...
        *,
        ...
        memory.last_own_damage_count = max(
        ...
        if obs.own_disabled:
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "DISABLED", "ACTOR_DISABLED"
            )
            return TacticalGoal(
                "DISABLED", memory.reason, "COAST", 0.0, 0.0, 0.0,
                False, 0.0, transition, memory.cycle,
            )

        # Terminal story intent has higher lifecycle priority than recovery.
        if story in {"HOLD", "SETTLE"} or phase == "PAYOFF":
            GenericBattleTacticalPlanner._settle(memory)
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "HOLD", "STORY_HIGH_LEVEL_HOLD"
            ) or transition
            return TacticalGoal(
                "HOLD", memory.reason, "BRAKE", 0.0, 0.0, 0.0,
                False, 1.0, transition, memory.cycle,
            )

        runway_required = (
            GenericBattleTacticalPlanner.engagement_runway_required(obs)
            if obs.requires_contact
            else 0.0
        )
        if obs.requires_contact:
            memory.engagement_runway_required_m = runway_required

        if new_contact or own_new_damage or new_damage:
            reverse_frames, turn_frames = GenericBattleTacticalPlanner._break_frames(obs)
            memory.cycle += 1
            memory.break_until_frame = max(
                int(memory.break_until_frame), int(obs.frame) + reverse_frames
            )
            memory.reposition_duration_frames = turn_frames
            memory.reposition_until_frame = memory.break_until_frame + turn_frames
            memory.separation_required_m = max(
                GenericBattleTacticalPlanner._recovery_separation_required(obs),
                runway_required,
            )
            memory.separation_achieved = False
            memory.engagement_runway_armed = False
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "BREAK_CONTACT", "REALIZED_CONTACT_OR_DAMAGE"
            )

        if memory.cycle > 0 and not memory.separation_achieved:
            if obs.surface_gap_m >= memory.separation_required_m:
                memory.separation_achieved = True
                memory.engagement_runway_armed = True
                memory.reposition_until_frame = int(obs.frame) + max(
                    2, int(memory.reposition_duration_frames)
                )
                transition = GenericBattleTacticalPlanner._set_mode(
                    memory, "REPOSITION", "RUNWAY_SEPARATION_CONFIRMED"
                ) or transition
            else:
                transition = GenericBattleTacticalPlanner._set_mode(
                    memory, "BREAK_CONTACT", "RUNWAY_SEPARATION_NOT_YET_CONFIRMED"
                ) or transition
                return TacticalGoal(
                    "BREAK_CONTACT", memory.reason, "REVERSE",
                    clamp(base_scale * 0.72, 0.18, 0.75),
                    0.0, bias * 0.18, False, 0.0, transition, memory.cycle,
                    stand_off_surface_gap_m=max(0.0, memory.separation_required_m),
                )

        if (
            memory.cycle > 0
            and memory.separation_achieved
            and obs.frame <= memory.reposition_until_frame
        ):
            # Revalidate geometry continuously; never let a timer latch stale spacing.
            if obs.surface_gap_m < memory.separation_required_m * 0.92:
                memory.separation_achieved = False
                memory.engagement_runway_armed = False
                transition = GenericBattleTacticalPlanner._set_mode(
                    memory, "BREAK_CONTACT", "RUNWAY_COLLAPSED_DURING_REPOSITION"
                ) or transition
                return TacticalGoal(
                    "BREAK_CONTACT", memory.reason, "REVERSE",
                    clamp(base_scale * 0.72, 0.18, 0.75),
            ...
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "REPOSITION", "POST_CONTACT_RUNWAY_HOLD"
            ) or transition
            return TacticalGoal(
                "REPOSITION", memory.reason, "ACCELERATE",
                clamp(base_scale * 0.52, 0.18, 0.68),
                0.0, bias * 0.65, False, 0.0, transition, memory.cycle,
                stand_off_surface_gap_m=max(0.0, memory.separation_required_m),
            )

        # The same generic rule applies to the first attack and every re-engagement.
        if obs.requires_contact and not memory.engagement_runway_armed:
            if obs.surface_gap_m >= runway_required:
                memory.engagement_runway_armed = True
            else:
                return GenericBattleTacticalPlanner._open_distance(
                    memory, base_scale, transition, "PRECONTACT_RUNWAY_REQUIRED"
                )

        if story in {"EVADE", "REGROUP", "REVERSE"}:
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "EVADE", "STORY_HIGH_LEVEL_EVASION"
            ) or transition
            return TacticalGoal(
                "EVADE", memory.reason,
                "REVERSE" if abs(obs.heading_error_rad) < 0.75 else "ACCELERATE",
                clamp(base_scale * 0.62, 0.18, 0.72),
                -1.15, bias * 0.65, False, 0.0, transition, memory.cycle,
            )

        if health_pressure > 0.28 and phase not in {"CLIMAX", "COUNTERATTACK"}:
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "EVADE", "DAMAGE_STATE_DISADVANTAGE"
            ) or transition
            return TacticalGoal(
                "EVADE", memory.reason, "ACCELERATE",
                clamp(base_scale * 0.58, 0.16, 0.66),
                -1.00, bias * 0.85, False, 0.0, transition, memory.cycle,
            )

        if phase == "COUNTERATTACK" or story == "COUNTER":
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "COUNTER", "CAUSAL_COUNTERATTACK_WINDOW"
            ) or transition
            return TacticalGoal(
                "COUNTER", memory.reason, "ACCELERATE",
                clamp(base_scale * 0.92, 0.32, 1.0),
                -0.10, bias * 0.30, bool(memory.engagement_runway_armed),
                0.0, transition, memory.cycle,
            )

        if (
            story in {"FLANK", "SURROUND"}
            or obs.contention >= 0.48
            or abs(obs.heading_error_rad) > 0.95
        ):
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "FLANK", "GEOMETRY_OR_CONTENTION"
            ) or transition
            lateral = 0.90 if story != "SURROUND" else 1.20
            return TacticalGoal(
                "FLANK", memory.reason, "ACCELERATE",
                clamp(base_scale * 0.78, 0.24, 0.92),
                -0.18, bias * lateral, False, 0.0, transition, memory.cycle,
            )

        stopping_distance = (
            max(0.0, obs.closing_speed_mps) ** 2
            / max(0.5, 2.0 * obs.own_braking_mps2)
        )
        geometric_buffer = max(
            0.35, 0.18 * (obs.own_length_m + obs.target_length_m)
        )
        if (
            obs.requires_contact
            and obs.surface_gap_m > 0.0
            and stopping_distance > obs.surface_gap_m + geometric_buffer
        ):
            transition = GenericBattleTacticalPlanner._set_mode(
                memory, "BRAKE_APPROACH", "BRAKING_DISTANCE"
            ) or transition
            strength = clamp(
                stopping_distance / max(0.2, obs.surface_gap_m + geometric_buffer),
                0.5,
                1.8,
            )
            return TacticalGoal(
                "BRAKE_APPROACH", memory.reason, "BRAKE", 0.0, 0.0, 0.0,
                False, strength, transition, memory.cycle,
            )

        transition = GenericBattleTacticalPlanner._set_mode(
            memory, "ENGAGE", "LIVE_STATE_ENGAGEMENT"
        ) or transition
        contact_commit = bool(
            obs.requires_contact
            and memory.engagement_runway_armed
            and abs(obs.heading_error_rad) <= 0.70
            and obs.contention < 0.80
        )
        return TacticalGoal(
