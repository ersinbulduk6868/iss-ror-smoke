...
def _record_precontact_sample(
    ...
    recovery_active = is_recovery_mode(str(obs and getattr(obs, "_controller_mode", "") or ""))
    # Real recovery authority lives in AutonomyMemory, not observation. Caller
    # clears samples before this function whenever memory.mode is RECOVER_*.
    if precontact_sample_eligible(
...
def _handoff_with_preserved_alignment(
    ...
    *,
    ...
    marker(
        ...
        evidenceFrame=int(sample.frame), evidenceSource="IMMEDIATELY_PREVIOUS_PRECONTACT_FRAME",
...
def c488_1_realized_handoff_update(
    ...
    *,
    ...
    if not previous_frame_sample_valid_for_handoff(
        sample,
        transaction_key=key,
...
def main() -> None:
    ...
    # C488 source-audit finding: continuous certificates were too permissive.
    # C488.1 replaces only C485's instantaneous handoff motion proof with a
    # one-frame, event-scoped precontact evidence bridge. C487 lifecycle and all
    # G05+ source/threshold authority remain untouched.
    ...
    print(json.dumps({
        ...
        "localityToleranceChanged":False,"damageAdmissionThresholdChanged":False,
        "assetIdentityBranch":False,"perAssetBattleCode":False,"perAssetTacticalTuning":False,
        "perVideoTrajectoryEngineering":False,"fixedWorldCoordinates":False,
        "exactCollisionFrameTarget":False,"exactImpactEnergyTarget":False,
        "actorPoseOrVelocityMutation":False,"fixtureBattlePlanChanged":False,
        "frozenNineServiceArchitectureChanged":False,"gateClosed":False,"productionReadyClaimed":False
    },sort_keys=True),flush=True))

    # C487.1 composition without executing superseded C488.
