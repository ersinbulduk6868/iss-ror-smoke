...
from blender import iss_battle_runtime_generic_battle_v5 as battle_v5
...
MECHANISM = "ISS_GENERIC_AUTONOMOUS_BATTLE_MECHANISM_V5"
AUDIT = "PRECONTACT_ENGAGEMENT_RUNWAY_AND_SURFACE_GAP_GEOMETRY_AUDIT_20260918"
...
def generic_battle_g06_outcome(states: dict[str, Any], events: dict[str, Any]) -> dict[str, Any]:
    ...
    if hardened._capture_output is None:
        raise RuntimeError("GENERIC_BATTLE_V5_OUTPUT_DIR_UNAVAILABLE")
    battle_v5.write_evidence(Path(hardened._capture_output))
    return outcome
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                "marker": "GENERIC_AUTONOMOUS_BATTLE_C463_ENGINEERING_READY",
                "candidate": CANDIDATE,
                "mechanism": MECHANISM,
                "affectedLayerAudit": AUDIT,
                "tacticalModel": TACTICAL_MODEL,
                "battleControlModel": battle_v5.BATTLE_CONTROL_MODEL,
                "battleSignalScope": BATTLE_SIGNAL_SCOPE,
                "engagementRunwayModel": ENGAGEMENT_RUNWAY_MODEL,
                "surfaceGapGoalModel": battle_v5.SURFACE_GAP_GOAL_MODEL,
