...
def solver_contact_simulate(scene: bpy.types.Scene, a: dict, b: dict) -> dict:
    ...
    for frame in range(duel.START, duel.END + 1):
        ...
        if frame >= duel.HANDOFF and impact is None:
            ...
            # Bullet resolves contact before the frame transform is exposed. At the
            # first overlapping/contact frame, current-frame velocity is already the
            # post-contact response. Therefore gate impact using the solver contact
            # geometry at this frame and the immediately preceding incoming velocity.
