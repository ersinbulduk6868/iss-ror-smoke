#!/usr/bin/env bash
set -euxo pipefail
mkdir -p artifacts/logs artifacts/logs/stall-diagnostics "$ROR_TEST_HOME"
...
ROR_PID=$!
cd "$GITHUB_WORKSPACE"
printf 'ROR_PID=%s\n' "$ROR_PID" | tee artifacts/logs/ror-pid.txt
...
grep -F 'ISS_MOTION_PROBE_V6_SCRIPT_MAIN' "$ANGELSCRIPT_LOG" || {
  cat "$ANGELSCRIPT_LOG" >&2
...
probe_count() {
  grep -F -c 'ISS_MOTION_PROBE_V6 seq=' "$ANGELSCRIPT_LOG" || true
}

capture_live_stall_diagnostics() {
  local label="$1" out="artifacts/logs/stall-diagnostics/$1" sample task tid gdb_rc
  set +e
  mkdir -p "$out"
  {
    echo "label=$label"
    echo "ror_pid=$ROR_PID"
    echo "monotonic_ns=$(monotonic_ns)"
    echo "utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo "probe_count=$(probe_count)"
    echo "ror_exe=$(readlink -f /proc/$ROR_PID/exe 2>/dev/null || true)"
  } > "$out/context.txt"
  kill -0 "$ROR_PID" 2>/dev/null || { echo 'ROR_NOT_ALIVE_AT_STALL_CAPTURE=1' >> "$out/context.txt"; set -e; return 0; }
  echo 'ROR_ALIVE_AT_STALL_CAPTURE=1' >> "$out/context.txt"

  ps -o pid,ppid,psr,pcpu,pmem,stat,etime,wchan:40,comm,args -p "$ROR_PID" > "$out/ps-process.txt" 2>&1 || true
  for sample in 1 2 3; do
    ps -L -p "$ROR_PID" -o pid,tid,psr,pcpu,stat,etime,wchan:40,comm > "$out/ps-threads-${sample}.txt" 2>&1 || true
    top -H -b -n 1 -p "$ROR_PID" > "$out/top-threads-${sample}.txt" 2>&1 || true
    [[ "$sample" -lt 3 ]] && sleep 1
  done

  for f in status stat sched limits maps; do
    sudo -n cat "/proc/$ROR_PID/$f" > "$out/proc-${f}.txt" 2>&1 || true
  done
  sudo -n cat "/proc/$ROR_PID/wchan" > "$out/proc-wchan.txt" 2>&1 || true
  sudo -n cat "/proc/$ROR_PID/syscall" > "$out/proc-syscall.txt" 2>&1 || true

  mkdir -p "$out/tasks"
  for task in /proc/"$ROR_PID"/task/*; do
    [[ -d "$task" ]] || continue
    tid="$(basename "$task")"
    {
      echo "TID=$tid"
  ...
  timeout 25s sudo -n gdb -q -batch \
    ...
    -p "$ROR_PID" > "$out/gdb-thread-apply-all-bt-full.txt" 2>&1
  gdb_rc=$?
  set -e
  printf 'gdb_rc=%s\n' "$gdb_rc" >> "$out/context.txt"
  kill -0 "$ROR_PID" 2>/dev/null && echo 'ROR_ALIVE_AFTER_GDB=1' >> "$out/context.txt" || echo 'ROR_ALIVE_AFTER_GDB=0' >> "$out/context.txt"
  cp "$ANGELSCRIPT_LOG" "$out/Angelscript-at-stall.log" 2>/dev/null || true
  cp "$ROR_LOG" "$out/RoR-at-stall.log" 2>/dev/null || true
  cp "$RUNNER_TEMP/ror-runtime.stdout.log" "$out/ror-runtime.stdout-at-stall.log" 2>/dev/null || true
}
  ...
  for poll in $(seq 1 "$PROBE_ACK_POLLS"); do
    ...
    if (( now > before )); then
      while IFS= read -r line; do
        [[ "$line" == *'ISS_MOTION_PROBE_V6 seq='* ]] || continue
...
m=re.search(r'ISS_MOTION_PROBE_V6 seq=(\d+) x=([-+0-9.eE]+) y=([-+0-9.eE]+) z=([-+0-9.eE]+) speed=([-+0-9.eE]+) accel=([01])', line)
if not m:
...
PY
        ...
        return 0
      done < <(grep -F 'ISS_MOTION_PROBE_V6 seq=' "$ANGELSCRIPT_LOG" | tail -n "+$((before + 1))")
    fi
  done
  printf 'label=%s state=ACK_TIMEOUT before=%s required_accel=%s monotonic_ns=%s\n' "$label" "$before" "$required_accel" "$(monotonic_ns)" >> artifacts/logs/probe-state-history.txt
  capture_live_stall_diagnostics "$label"
...
assert_focus
for readiness_index in $(seq 1 "$RUNTIME_READINESS_SAMPLES"); do
  wait_fresh_probe "readiness-${readiness_index}" 0
  assert_probe_accel "artifacts/logs/position-readiness-${readiness_index}.txt" 0
done
python3 - "$RUNTIME_READINESS_SAMPLES" > artifacts/runtime-readiness-evidence.txt <<'PY'
import re,sys
from pathlib import Path
n=int(sys.argv[1])
seqs=[]; times=[]
for i in range(1,n+1):
    text=Path(f'artifacts/logs/position-readiness-{i}.txt').read_text()
    seqs.append(int(re.search(r'^PROBE_SEQ=(\d+)$',text,re.M).group(1)))
    times.append(int(re.search(r'^MONOTONIC_NS=(\d+)$',text,re.M).group(1)))
if any(b<=a for a,b in zip(seqs,seqs[1:])):
    raise SystemExit(f'RUNTIME_READINESS_GATE_FAIL: non-increasing probe sequence {seqs}')
if any(b<=a for a,b in zip(times,times[1:])):
    raise SystemExit(f'RUNTIME_READINESS_GATE_FAIL: non-increasing monotonic timestamps {times}')
print(f'RUNTIME_READINESS_SAMPLES={n}')
print('RUNTIME_READINESS_SEQS='+','.join(map(str,seqs)))
print('RUNTIME_READINESS_GATE=CONSECUTIVE_FRESH_FRAMESTEP_PASS')
PY
cat artifacts/runtime-readiness-evidence.txt
