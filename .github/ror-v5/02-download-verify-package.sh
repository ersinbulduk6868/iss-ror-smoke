...
void main()
{
    log("ISS_MOTION_PROBE_V5_SCRIPT_MAIN");
}
...
void frameStep(float dt)
{
    ...
    log("ISS_MOTION_PROBE_V5 seq=" + g_probe_seq
        + " x=" + pos.x + " y=" + pos.y + " z=" + pos.z
        + " speed=" + actor.getSpeed() + " accel=" + accel);
    g_probe_seq++;
}
AS
cp "$PROBE_SCRIPT" artifacts/logs/iss_motion_probe_v5.as
test -s "$PROBE_SCRIPT"
grep -F 'ISS_MOTION_PROBE_V5_SCRIPT_MAIN' "$PROBE_SCRIPT"
